...
export const setupAvtotoPartsCommand = (bot: Telegraf) => {
  ...
  bot.on('text', async (ctx) => {
    try {
      ...
      const manufacturers = new Set(partsResult.Parts.map((part: any) => part.Manuf));
      const buttons = Array.from(manufacturers).map((manuf) => {
        const parts = partsResult.Parts.filter((part: any) => part.Manuf === manuf);
  ...
  bot.action(/manuf:(.+)/, async (ctx) => {
    try {
      ...
      const selectedParts = partsResult.Parts
        .filter((part: any) => part.Manuf === selectedManuf)
      ...
      const detailButtons: InlineButton[] = selectedParts.map((part: any, index: number) => {
        ...
        return { text: buttonText, callback_data: `detail:${selectedManuf}:${index}` };
      });
  ...
  bot.action(/detail:(.+):(.+)/, async (ctx) => {
    try {
      ...
      await ctx.reply(messageText, {
        reply_markup: {
          inline_keyboard: [
            [{ text: "✅ Создать заказ", callback_data: `create_order:${selectedManuf}:${index}` }]
          ]
        }
      });
      await ctx.answerCbQuery();
  ...
  bot.action(/create_order:(.+):(.+)/, async (ctx) => {
    try {
      ...
      const part = selectedParts[index];
      // Предположим, что деталь имеет свойство Articule, используем его для создания заказа; если отсутствует – используем название
